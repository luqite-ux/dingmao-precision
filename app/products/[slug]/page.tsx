...
export default async function ProductDetail({ params }: Props) {
  ...
  if (!product) notFound()
  return <><section className="product-detail-hero"><div className="product-detail-copy"><Link href="/products"><ArrowLeft size={16} /> All products</Link><span className="eyebrow">{product.categoryName}</span><h1>{product.name}</h1><p>{product.description}</p><Link className="button primary" href={product.inquiryHref}>Request this component <ArrowRight size={17} /></Link></div><div className="product-detail-image"><Image src={product.image} alt={product.name} fill sizes="(max-width:800px) 100vw, 50vw" priority /></div></section>
